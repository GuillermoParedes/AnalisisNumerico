...
function metodoSeidel() {
 ...
 for (var iii = 0; iii <=(gjNsize-1); iii++){
  ...
  finversa();
  printInversa();
  // printIdentidad(identidad);
  printMatriz(identidad,'#iden');
  MatrizM();
  printPasos();
 ...
 function MatrizM(){
  var m1 = valoresMatriz();
  var m2 = valoresMatriz();
...
// PREPARANDO LA MATRIZ M
var aux  = new array(tam);
  for (var i = 0 ;i < tam; i++){
    for (var j = 0; j <  tam+1; j++){
      if(i != j){
        aux = (m1[i][j]/m1[i][j-1]);
...
function printIdentidad(valor){
 $('#iden').html( function(){
  var es = "<table><tbody>";
        for (var i = 0; i < gjNsize; i++) {
         es += "<tr>";
         for (var j = 0; j < gjNsize; j++) {
          es += "<td class='matel'>"+ Math.round(valor[i][j] * 100) / 100 + "</td>"
...
function printMatriz(valor,idvalor){
 $(idvalor).html( function(){
  var es = "<table><tbody>";
        for (var i = 0; i < gjNsize; i++) {
         es += "<tr>";
         for (var j = 0; j < gjNsize; j++) {
